...
for strategist in [OccurrenceBasedStrategist, RandomStrategist, MixedStrategist]:
    ...
    for word in tqdm(answer_word_list):
        helper = strategist()
        a = helper.give_suggestion_from_obs('', [], '', [], '', [])
        # print(a[0][0])
        answer = WordleAgent(word)
        #answer = WordleAgent('watch')
        for i in range(6):
            
            black_char, black_id, yellow_char, yellow_id, green_char, green_id = answer.attempt_word(a)
            ...
            a = helper.give_suggestion_from_obs(black_char, black_id, yellow_char, yellow_id, green_char, green_id)
